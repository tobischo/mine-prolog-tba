...
printPossiblePaths:-position(X),writeln('You can go to the following areas: '),connection(X,Y),writeln(Y).
...
%look around
lookAround:-position(X),viewable(X,Y),writeln('You can see:'),writeln(Y),false.

%viewable
viewable(X,Y):-contains(Y,X).

%examine
examine(Y):-position(X),viewable(X,Y),viewable(Y,Z),write(Z),false.
...
%move action
go_to(X):-position(Y),mvR(X,Y)->retract(position(Y)),asserta(position(X)),printPos;write('Can not move to '),write(X).
